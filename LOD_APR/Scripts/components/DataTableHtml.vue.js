                    ...
                    <div class="col-md-12">
                        ...
                        <ul class="pagination m-t-xs pull-right">
                            ...
                            <li v-for="index in ListPaginas" :class="[{ active: PaginaActual==index }, 'paginate_button']">
                                <a role="button" v-if="PaginaActual == index" :style="BtnColor" @click="$emit('click',index)">{{index}}</a>
                                <a role="button" v-if="index > PaginaActual || index < PaginaActual" @click="$emit('click',index)">{{index}}</a>
                            </li>
...
Vue.component('button-submit-filter', {
    ...
    template: `<button class="btn btn-sm btn-group-justified" data-style="zoom-in" type="submit" @click='onSubmit'><i class="fa fa-filter"></i> Filtrar</button>`
})